...
void AppClass::InitVariables(void)
{
	...
	// createing realigned bounding boxes for all models
	ReAlignedBox1 = new MyRealignedBoundingBoxClass(m_pBS0->GetVerticies());
	ReAlignedBox2 = new MyRealignedBoundingBoxClass(m_pBS1->GetVerticies());
	ReAlignedBox3 = new MyRealignedBoundingBoxClass(m_pBS2->GetVerticies());
...
void AppClass::Update(void)
{
	...
	ReAlignedBox2->SetModelMatrix(m_pMeshMngr->GetModelMatrix("Steve"));
	ReAlignedBox2->RenderSphere();

	m_pBS2->SetModelMatrix(m_pMeshMngr->GetModelMatrix("Cow"));
	m_pBS2->RenderSphere();

	ReAlignedBox3->SetModelMatrix(m_pMeshMngr->GetModelMatrix("Cow"));
	ReAlignedBox3->RenderSphere();

	m_pBS0->SetColliding(false);
	m_pBS1->SetColliding(false);
	m_pBS2->SetColliding(false);

	ReAlignedBox1->SetColliding(false);
	ReAlignedBox2->SetColliding(false);
	ReAlignedBox3->SetColliding(false);
	...
	ReAlignedBox1->SetModelMatrix(m_pMeshMngr->GetModelMatrix("Zombie"));
	ReAlignedBox1->RenderSphere();//render the bounding sphere
