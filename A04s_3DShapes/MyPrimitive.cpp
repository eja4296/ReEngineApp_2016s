...
void MyPrimitive::GenerateSphere(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	for (int i = 0; i < nVert; i++)
	{
		...
		right.push_back(static_cast <vector3>(rotation * glm::vec4(m_lVertexPos[i], 1.0f))); // ,1.0 makes it a vec4 just to multiply, then have to cast back to vec3 to add it to the list
	}
